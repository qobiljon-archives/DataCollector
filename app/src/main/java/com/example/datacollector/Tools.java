...
class Tools {
    ...
    static boolean isNetworkUnavailable() {
    ...
    static synchronized int post(Context context, @StringRes int api, Pair<String, String> parameter) throws MalformedURLException, InterruptedException, ExecutionException {
        if (isNetworkUnavailable())
            return -1;

        CallAPI callAPI = new CallAPI(context, api);
    ...
    private static class CallAPI extends AsyncTask<String, Void, Integer> {
        CallAPI(Context context, @StringRes int api) throws MalformedURLException {
